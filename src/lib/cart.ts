...
export function addToCart(
  ...
  // Check the combined quantity, not just this call's qty, so topping up an
  // existing line cannot go past what is on hand either.
...
export function discountCents(
  ...
  if (coupon.type === "percent") {
    // A percent coupon is a share of the subtotal, not a flat cent amount.
    // roundCents keeps the result on a whole cent, the same guard taxCents
    // already applies, so 10% of $19.99 cannot leave a fraction behind.
